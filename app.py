# app.py - GannXPro COMPLETE with Live Market Updates & Date-wise Option Chain
...
# ----------- DATE-WISE OPTION CHAIN FUNCTIONS ----------- #
def generate_expiry_dates(symbol):
    """Generate realistic expiry dates for options"""
    today = datetime.now()
    
    if symbol in ['NIFTY', 'BANKNIFTY']:
        # For indices: Weekly expiries (Thursday) + Monthly
        expiries = []
        
        # Current month last Thursday
        current_month = today.replace(day=28)
        while current_month.weekday() != 3:  # Thursday
            current_month += timedelta(days=1)
        if current_month >= today:
            expiries.append(current_month.strftime("%d-%b-%Y"))
        
        # Next weekly expiries (next 3 Thursdays)
        for i in range(1, 4):
            next_thursday = today + timedelta(days=(3 - today.weekday() + 7 * i) % 7)
            if next_thursday.strftime("%d-%b-%Y") not in expiries:
                expiries.append(next_thursday.strftime("%d-%b-%Y"))
        
        # Next month expiry
        next_month = (today.replace(day=28) + timedelta(days=7)).replace(day=28)
        while next_month.weekday() != 3:
            next_month += timedelta(days=1)
        if next_month.strftime("%d-%b-%Y") not in expiries:
            expiries.append(next_month.strftime("%d-%b-%Y"))
        
        return expiries[:5]  # Return max 5 expiries
    
    else:
        # For stocks: Monthly expiries only (last Thursday)
        expiries = []
        for i in range(3):
            month_date = today.replace(day=28) + timedelta(days=31 * i)
            while month_date.weekday() != 3:
                month_date += timedelta(days=1)
            if month_date >= today:
                expiries.append(month_date.strftime("%d-%b-%Y"))
        
        return expiries[:3]  # Return max 3 expiries

def generate_date_wise_option_chain(symbol, selected_expiry=None):
    """Generate date-wise option chain with different characteristics per expiry"""
    try:
        base_prices = {
        ...
        expiries = generate_expiry_dates(symbol)
        
        if not selected_expiry:
            selected_expiry = expiries[0]  # Default to nearest expiry
        
        all_option_data = {}
        
        for expiry in expiries:
            # Different characteristics based on expiry
            days_to_expiry = (datetime.strptime(expiry, "%d-%b-%Y") - datetime.now()).days
            time_decay_factor = max(0.1, days_to_expiry / 30)  # More time = higher premiums
            
            # Generate strikes
            strikes = []
            strike_step = 100 if symbol in ['NIFTY', 'BANKNIFTY'] else 50
            
            for i in range(-8, 9):  # More strikes for better analysis
            ...
            for strike in strikes:
                distance_factor = abs(strike - base_price) / base_price
                ...
                expiry_data.append({
                    ...
                    'strike': strike,
                    'expiry': expiry,
                    'oi': max(500, int(15000 / (distance_factor + 0.1) * time_decay_factor * np.random.uniform(0.7, 1.3))),
                    'volume': max(50, int(2000 / (distance_factor + 0.1) * time_decay_factor * np.random.uniform(0.6, 1.4))),
                    'iv': max(10, 20 + (distance_factor * 100 * np.random.uniform(0.8, 1.2))),
                    'ltp': max(1, abs(strike - base_price) * 0.15 * time_decay_factor * np.random.uniform(0.7, 1.3)),
                ...
                expiry_data.append({
                    ...
                    'strike': strike,
                    'expiry': expiry,
                    'oi': max(500, int(18000 / (distance_factor + 0.1) * time_decay_factor * np.random.uniform(0.7, 1.3))),
                    'volume': max(50, int(2500 / (distance_factor + 0.1) * time_decay_factor * np.random.uniform(0.6, 1.4))),
                    'iv': max(10, 22 + (distance_factor * 100 * np.random.uniform(0.8, 1.2))),
                    'ltp': max(1, abs(strike - base_price) * 0.15 * time_decay_factor * np.random.uniform(0.7, 1.3)),
                    'change': np.random.uniform(-20, 20),
                    'days_to_expiry': days_to_expiry
                })
            
            all_option_data[expiry] = expiry_data
        
        # Calculate PCR and totals for selected expiry
        selected_data = all_option_data.get(selected_expiry, [])
        total_ce_oi = sum([item['oi'] for item in selected_data if item['type'] == 'CE'])
        total_pe_oi = sum([item['oi'] for item in selected_data if item['type'] == 'PE'])
...
def display_option_chain_table(option_data, selected_expiry):
    ...
    expiry_data = option_data['all_option_data'].get(selected_expiry, [])
    
    if not expiry_data:
        st.warning(f"No data available for expiry {selected_expiry}")
        return
    
    # Create DataFrame for the selected expiry
    df_data = []
    for option in expiry_data:
        df_data.append({
            ...
            'LTP': f"₹{option['ltp']:.2f}",
            'Change': f"{option['change']:+.1f}%",
            'OI': f"{option['oi']:,}",
            'Volume': f"{option['volume']:,}",
            'IV': f"{option['iv']:.1f}%",
            'Days to Expiry': option['days_to_expiry']
        })
    
    df = pd.DataFrame(df_data)
    
    # Split into Calls and Puts for better visualization
    calls_df = df[df['Type'] == 'CE'].drop('Type', axis=1)
    puts_df = df[df['Type'] == 'PE'].drop('Type', axis=1)
    ...
    with col1:
        ...
        st.dataframe(calls_df, use_container_width=True, height=400)
    
    with col2:
        st.markdown("**📉 PUT OPTIONS (PE)**")
        st.dataframe(puts_df, use_container_width=True, height=400)
...
# ----------- DATE-WISE OPTION CHAIN TAB ----------- #
elif app_mode == "Option Chain":
    st.markdown('<div class="section-header">🔗 Date-wise Option Chain Analysis</div>', unsafe_allow_html=True)
    ...
    # Get live option chain with date-wise data
    option_data = generate_date_wise_option_chain(selected_symbol)
    ...
    if option_data and option_data['success']:
        ...
        expiries = option_data['available_expiries']
        
        # Display expiry badges
        cols = st.columns(len(expiries))
        selected_expiry = option_data['selected_expiry']
        
        for i, expiry in enumerate(expiries):
            with cols[i]:
                if st.button(f"{expiry}", key=f"exp_{i}", use_container_width=True):
        ...
        col5.metric("Selected Expiry", selected_expiry)
        ...
        if pcr_value > 1.2:
            ...
            pcr_signal = "🟡 Neutral PCR"
        
        st.info(f"**PCR Signal**: {pcr_signal} - Put-Call Ratio: {pcr_value:.2f}")
        
        # Display the option chain table
        display_option_chain_table(option_data, selected_expiry)
        
        # Expiry-wise Analysis
        st.subheader("📈 Expiry-wise Analysis")
        
        analysis_cols = st.columns(len(expiries))
        for i, expiry in enumerate(expiries):
            with analysis_cols[i]:
                expiry_data = option_data['all_option_data'].get(expiry, [])
                if expiry_data:
                    ce_oi = sum([item['oi'] for item in expiry_data if item['type'] == 'CE'])
                    pe_oi = sum([item['oi'] for item in expiry_data if item['type'] == 'PE'])
                    expiry_pcr = pe_oi / ce_oi if ce_oi > 0 else 0
                    
                    days_to_expiry = expiry_data[0]['days_to_expiry']
